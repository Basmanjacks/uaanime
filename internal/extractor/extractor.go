...
// Для відтворення резолюцію імені (яке цілком може вказувати на приватну адресу),
// редиректи і абсолютні URL усередині HLS-плейлиста виконує зовнішній плеєр, і
// застосунок їх не контролює. Це свідомий залишковий ризик v1: перехопити той
// трафік можна лише локальним проксі — окремим мережевим компонентом,
// несумісним із «легким продуктом», а per-host allowlist CDN ламався б при
// кожній зміні хоста, тобто на найчастішому класі поломок provider-repair.
//
// Екстрактори m3u8 не парсять і після 2026-09-16: плейлист розбирає лише
// internal/download, і лише щоб зберегти серію на диск. Там межа довіри ширша —
// за URL із недовіреного плейлиста ходить сам застосунок, — тому завантажувач
// має власний перевірений dialer (internal/httpx, fetch.go/dial.go: deny-таблиця
// непублічних префіксів, dial у вже перевірену IP, перевірка на кожному хопі
// редиректу) і ніколи не бере проксі з оточення. Для відтворення все лишається
// як було: URL віддається плеєру, він за ними й ходить.
